...
MinoMasterMidiMapper::MinoMasterMidiMapper(MinoMaster *parent) :
    ...
    _master(parent),
    _program(NULL),
    _knobsPerTrack(4), // FIXME: Hardcoded value
...
void MinoMasterMidiMapper::registerRoles()
{
    ...
    for (int i=0; i<MINOMASTERMIDIMAPPER_TRACKS_MAX; i++)
    {
        ...
        role = QString("MASTER_ANIMATION_CC_%1").arg(i);
        description = QString("Toggle master's animation with CC #%1").arg(i);
...
void MinoMasterMidiMapper::updateMap()
{
    ...
    if (_program)
    {
        ...
        for(int i=_virtualPageOffset; i<range_max; ++i)
        {
            ...
            role = QString("MASTER_ANIMATION_CC_%1").arg(role_index);
            MidiMapper::connectControl(role, group, SLOT(setEnabledConnectControl(quint8)), true);
